...
You need Python 3.10 or later. Check with `python3 --version`; if you do not have
it, install from [python.org](https://www.python.org/downloads/) or via Homebrew.
...
That is the whole install, and it takes about a minute. Every package the course
uses ships as a prebuilt wheel for macOS, Linux and Windows, so nothing is
compiled and there is no dependency solve to wait on.
...
**Every time you come back to the course**, reactivate the environment first:
...
Your prompt will show `(.venv)`. Nearly every "it worked yesterday" problem is a
forgotten activation.

**Week 9 only.** The N-body integrator `rebound` is compiled from C source rather
than installed from a wheel, so it is kept out of the main install. You will not
need it until Week 9:
...
**If you prefer conda**, use `environment.yml` in place of the two `venv` lines:
...
That file lists only Python and pip — every course package is declared in
`pyproject.toml` and installed by `pip install -e .`. conda therefore has almost
nothing to solve and finishes in seconds, and there is only one dependency list
to maintain either way.

Reactivate with `conda activate astr5820` rather than `source .venv/bin/activate`;
everything else in this document is the same.
...
`pyproject.toml`, this is the same command whether you set up with `venv` or with
conda, and it never involves a solve.
...
|---|---|
| `ModuleNotFoundError: No module named 'astrotools'` | You skipped `pip install -e .`, or the environment is not active. Run `source .venv/bin/activate` from the repository root, then `pip install -e .`. |
| `ModuleNotFoundError` for numpy or scipy after installing them | Two Pythons. `python check_setup.py` prints the interpreter path — confirm it sits inside `.venv` in the repository. |
...
| Jupyter cannot see the environment | With `.venv` active, run `pip install -e ".[notebook]"` then `python -m ipykernel install --user --name astr5820`, and pick that kernel. |
